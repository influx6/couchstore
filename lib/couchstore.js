var em = require('em')('couchstore',function(em){
  ...
  couch.allDB = function(done,fail){
    return Promise.create(function(_self){
      nano(couch.urlConstruct(couch.defaults)).db.list(function(err,body,header){
          ...
          if(err || couch.validateFailHeader(header)) return _self.reject(res);
  ...
  couch.validateFailHeader = function(header){
     var status = header['status-code'];
  ...
  couch.use = function(name,config){
    ...
    db.all = function(){
      return this.view('all').then(function(res){
      ...
      return this.p.then(function(db){
        return Promise.create(function(_self){
          db.head(doc,function(err,body,header){
            if(err || couch.validateFailHeader(header)) return _self.reject(couch.responseFragment(err,body,header));
    ...
    db.get = function(doc,rev,donefn,errfn){
      return this.p.then(function(db){
        return Promise.create(function(_self){
          db.get(doc,function(err,body,header){
            if(err || couch.validateFailHeader(header)) return _self.reject(couch.responseFragment(err,body,header));
    ...
    db.getAttachment = function(doc,att,params,encoding){
      ...
      if(!self.ID.fetch(doc)) return this.info(doc).then(function(res){ return op(res.header.etag); }).promise();
      return this.p.then(function(){ return op(self.ID.fetch(doc)); }).promise();
    };
